...
class UIManager(IMainManagerAdapter):
    ...
    def update_settings_popup(self):
        ...
        if self.settings_popup:
            ...
            translation_server_url = self.main_manager.get_config("translation_server_url")
    ...
    def check_gui_server_status(self) -> Union[Tuple[bool, int], None]:
        self.main_manager.get_error_handler().log_message("UIManager.check_gui_server_status called")
        try:
    ...
    def set_launch_mode(self, mode: int):
        ...
        if result.success:
            self.main_manager.get_error_handler().log_message(f"Launch mode set to {mode}")
